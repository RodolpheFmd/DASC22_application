...
class ActorNetwork(nn.Module):
    def __init__(self):
        ...
        self.Fwd = nn.Sequential(
            nn.Linear(8, 256*4),
            nn.Tanh(),
            nn.Linear(256*4, len(set.commands)),
            nn.Softmax(dim=-1))
        self.device = T.device('cuda:0' if T.cuda.is_available() else 'cpu')
        self.to(self.device)
...
class SuperAgent:
    def __init__(self):
        ...
        print(f'\n\n--LOADING MODELS--\n')
        if set.os == 'L':
            self.Policy.load_state_dict(T.load(set.path_save+'Multi_agent/Actor_'+str(set.scenario)))
            self.gru.load_state_dict(T.load(set.path_save+'Multi_agent/Gru_'+str(set.scenario)))
        else:
            self.Policy.load_state_dict(T.load(set.path_save+'Multi_agent\\Actor_'+str(set.scenario)))
            self.gru.load_state_dict(T.load(set.path_save+'Multi_agent\\Gru_'+str(set.scenario)))
        self.Policy.eval()
